...
- [x] text atlas edge fix: the built-in 8x8 font now bakes each glyph in a 10x10 cell with a 1px transparent border, and emit_glyph samples texel centres (half-texel UV inset) -- so a glyph quad at a fractional scale / position no longer bleeds a neighbour's edge column or shows an MSAA seam (visible once mge_ui.c drew text scaled up).
