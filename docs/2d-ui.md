...
| kind | calls |
| --- | --- |
| lifecycle | `Mge_UiNewFrame(dt)` (lazy-boots; samples the mouse) / `Mge_UiRender` / `Mge_UiShutdown`; `Mge_UiSetRoot`, `Mge_UiViewport(w,h)` (0,0 → screen size); `Mge_UiWantsPointer` (true while the cursor is over a scroll view or a drag is active — gate your game's mouse handling on it) / `Mge_UiWantsKeyboard` (still `false`; Phase 3) |
