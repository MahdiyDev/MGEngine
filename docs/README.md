...
[Assimp](https://github.com/assimp/assimp). Assimp is C++, so the library is
linked with `g++` and bakes in the C/C++ runtimes — consumers stay pure C. The
editor's own UI ([Dear ImGui](https://github.com/ocornut/imgui) behind the C
`Mge_Gui*` abstraction, `editor/mge_gui.cpp`) is compiled into `editor.exe`, not
the shipped library.
...
| **[2d-ui.md](2d-ui.md)** | text rendering (`mge_text.c` — 2D + world-space), the editor-only `Mge_Gui*` ImGui abstraction, the `Mge_Ui*` retained widget GUI |
